...
function Map({isVisible,setIsVisible}){
    return(
        ...
        className='m-10 max-sm:col-span-3'>
            {isVisible ? <button className=' h-14 w-full bg-[#fc8019] text-white' 
            onClick={()=>{
            setIsVisible(false)
        }}>Select your address    
        </button>
        ...
        <button 
        className=' h-14  w-full border border-[#fc8019] ' 
        onClick={()=>{
            setIsVisible(true)
        }}>Select your address    
        </button>}
        {isVisible && 
        <div 
        className='h-[85vh] shadow-2xl'>
            <div>
            <div 
            className='font-bold text-2xl m-3'
            >Save delivery address</div>
            ...
            className='m-2' >
                ...
                className='border border-gray-400 h-[70px] w-[100%] text-[17px] px-[20px] pt-[22px] box-border whitespace-nowrap overflow-x-hidden text-ellipsis font-semibold' 
                />
            ...
            className='m-2'>
                ...
                <button 
                className=' h-14 w-[90%] m-5  bg-[#fc8019] text-white'
                >SAVE ADDRESS & PROCEED</button>
            </div>
        </div>}
        </div>
    )
...
function Payment({isVisible,setIsVisible}){
    return (
        <div className='m-10 max-sm:col-span-3'>
            {isVisible ?
            <button 
            className='border h-14 w-full bg-[#fc8019] text-white' 
            onClick={()=>{
                setIsVisible(false)
            }}>Choose payment method   
            </button> :
            <button 
            className='border border-[#fc8019] h-14 w-full ' 
            onClick={()=>{
                setIsVisible(true)
            }}>Choose payment method  
            </button>}
            ...
            <div 
            className='h-[40vh] grid grid-cols-4 '>
